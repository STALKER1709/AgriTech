<?php

declare(strict_types=1);

namespace App\Livewire\Client;

use App\Models\Conversation;
use App\Models\Message;
use App\Models\User;
use App\Services\Messaging\MessagingService;
use DomainException;
use Flux\Flux;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\Auth;
use Livewire\Attributes\Title;
use Livewire\Attributes\Url;
use Livewire\Component;

/**
 * One conversation, seen by the client.
 *
 * Opening the thread marks the other side's messages read — only the
 * participant's own view does that, and only for messages addressed to them.
 * Polling keeps the thread fresh without a WebSocket server, which is what
 * the local-only constraint asks for.
 */
#[Title('Conversation')]
class MessageThread extends Component
{
    public Conversation $conversation;

    public string $content = '';

    #[Url]
    public string $after = '';

    public function mount(Conversation $conversation): void
    {
        $user = Auth::user();

        abort_unless($user instanceof User && $conversation->includes($user), 403);

        $this->conversation = $conversation->load(['client.farmerProfile', 'farmer.farmerProfile']);

        app(MessagingService::class)->markThreadRead($conversation, $user);
    }

    /**
     * @return Collection<int, Message>
     */
    public function messages(): Collection
    {
        return $this->conversation->messages()->with('sender')->orderBy('id')->get();
    }

    /**
     * The messages grouped by day, for the date separators the mockup draws:
     * « Aujourd'hui », « Hier », then the date itself.
     *
     * @return Collection<string, Collection<int, Message>>
     */
    public function days(): Collection
    {
        return $this->messages()->groupBy(function (Message $message): string {
            $at = $message->created_at?->timezone(config('app.timezone'));

            if ($at === null) {
                return (string) __('Sans date');
            }

            return match (true) {
                $at->isToday() => (string) __('Aujourd\'hui'),
                $at->isYesterday() => (string) __('Hier'),
                default => $at->translatedFormat('d F Y'),
            };
        });
    }

    /**
     * The other side of the thread, whose name the header carries.
     */
    public function counterpart(): User
    {
        return $this->conversation->farmer;
    }

    public function send(MessagingService $messaging): void
    {
        $user = Auth::user();

        abort_unless($user instanceof User, 403);

        $this->validate([
            'content' => ['required', 'string', 'min:1', 'max:5000'],
        ], [], [
            'content' => __('message'),
        ]);

        try {
            $messaging->send($this->conversation, $user, $this->content);
        } catch (DomainException $exception) {
            Flux::toast(variant: 'danger', text: $exception->getMessage());

            return;
        }

        $this->reset('content');
    }

    public function render(): mixed
    {
        return view('livewire.client.message-thread');
    }
}
